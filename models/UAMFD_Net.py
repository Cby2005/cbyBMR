...
class UAMFD_Net(nn.Module):
    def __init__(self,
        ...
        model_name = os.environ.get('BMR_BERT_CHINESE', 'bert-base-chinese') if self.dataset not in english_lists \
            else os.environ.get('BMR_BERT_UNCASED', 'bert-base-uncased')
    ...
    def train(self, mode=True):
        super(UAMFD_Net, self).train(mode)
        if self.paper_strict:
            # Frozen pretrained encoders should not inject train-time dropout noise.
