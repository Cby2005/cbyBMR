...
token_chinese = BertTokenizer.from_pretrained(os.environ.get('BMR_BERT_CHINESE', 'bert-base-chinese'))
token_uncased = BertTokenizer.from_pretrained(os.environ.get('BMR_BERT_UNCASED', 'bert-base-uncased'))
...
def main(args):
    ...
    LIST_ALLOW_AMBIGUITY = ['gossip','weibo','Weibo_21']
    ...
    setting['paper_strict'] = os.environ.get('BMR_PAPER_STRICT', '0') == '1'
    print("Paper-strict protocol: {}".format(setting['paper_strict']))
    ######## ADDITIONAL FEATURES ###########
    ...
    ########## train dataset ####################
    if setting['train_dataname'] in ['weibo', 'Weibo_21']:
        print("Using {} as training".format(setting['train_dataname']))
        # Note: bert-base-chinese is within MixSet_dataset
        from data.weibo_dataset import weibo_dataset
        train_dataset = weibo_dataset(dataset=setting['train_dataname'], is_train=True, image_size=GT_size,
    ...
    #         shuffle=True)
        ...
        train_loader = DataLoader(train_dataset, batch_size=custom_batch_size, shuffle=True, collate_fn=collate_fn_english,
                                  ...
                                  pin_memory=True)
        setting['thresh'] = 0.80 if setting['paper_strict'] and setting['train_dataname'] == 'gossip' else train_dataset.thresh
        print(f"thresh:{setting['thresh']}")


    ########## validate dataset ####################
    if setting['val_dataname'] in ['weibo', 'Weibo_21']:
        print("Using {} as inference".format(setting['val_dataname']))
        from data.weibo_dataset import weibo_dataset
    ...
    if setting['paper_strict']:
        # Paper: frozen MAE/BERT and Adam with one 1e-4 learning rate.
        strict_params = [p for p in model.parameters() if p.requires_grad]
        strict_optimizer = torch.optim.Adam(strict_params, lr=1e-4)
        strict_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(strict_optimizer, T_max=num_steps)
        optimizer = optimizer_fast = optimizer_extremefast = None
        scheduler = scheduler_fast = scheduler_extremefast = None
        warmup_scheduler = warmup_scheduler_fast = warmup_scheduler_extremefast = None
        print("Using paper optimizer: Adam(lr=1e-4)+CosineAnnealingLR")
    else:
        strict_optimizer = strict_scheduler = None
        ...
        print("Using released-code AdamW groups+CosineAnnealingLR+UntunedLinearWarmup")
    #############################################################
        ...
        if setting['val']==0:
            ...
            for i, items in enumerate(train_loader):
               ...
               (input_ids1, attention_mask1, token_type_ids1), (image1, labels1, sents1)
                    ...
                    cc_weight = 4.0 if setting['paper_strict'] else 2.0
                    loss = loss_CE + cc_weight * loss_ambiguity + 1.0 * loss_single_modal
                    # if use_scalar:
...
def evaluate(validate_loader, model, criterion, progbar=None, setting={}):
    ...
    # The paper fixes thresholds by dataset; the released code otherwise scans on evaluation data.
    THRESH = [threshold] if setting.get('paper_strict', False) else [threshold+i/500 for i in range(-20,20)]
